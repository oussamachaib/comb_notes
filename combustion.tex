...
\begin{center}
\fbox{$\underbrace{u'}_{turbulence} \xrightarrow{influences} s_T \xrightarrow{influences} \underbrace{\Omega_r}_{global\, reaction\, rate}
	$}
\end{center}
\vspace{5pt}
...
\begin{center}
	\fbox{$\frac{s_T}{s_L} = \underbrace{\frac{A_T}{A_L}}_{\sim\,wrinkling}$}
\end{center}
We can also say that: 
\[\frac{A_T}{A_L}\sim \underbrace{l_t}_{integral\,length\,scale} \sim \underbrace{\mu_t}_{turbulent\,viscosity} \sim Re_t
\]
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\linewidth]{figures/turbulence.png}
	\caption{Evolution of a non-premixed flame jet with increasing turbulence (Hottel and Hawthorne 1949)}
\end{figure}
Moral of the story:
\begin{center}
	\fbox{$\underbrace{u'}_{turbulence} \xleftrightarrow{influences} s_T \xleftrightarrow{influences} \underbrace{\Sigma}_{flame\,surface\,density} \xleftrightarrow{influences} \underbrace{\Omega_r}_{global\, reaction\, rate}
		$}
\end{center}
\subsection{Favre and Reynolds averaging}
Clear so far, I'll write something when I have the time.
